...
export async function createNeteaseQrUnikey() {
    const body = weapiEncrypt({ type: 1 });
    const { data } = await axios.post(
...
export async function checkNeteaseQrStatus(
    ...
    const body = weapiEncrypt({ key: unikey, type: 1 });
    const response = await axios.post(
    ...
    if (code === 803) {
        const newCookie = parseSetCookie(response.headers);
        const old = getNeteaseAuth()?.cookie || "";
        let cookie = mergeCookie(old, newCookie);
        // 部分环境 cookie 在 body
        if (data.cookie) {
            cookie = mergeCookie(cookie, String(data.cookie));
        }
        if (!cookie) {
