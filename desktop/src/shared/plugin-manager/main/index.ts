...
class PluginManager {
    ...
    public async setup(windowManager: IWindowManager) {
        ...
        // 3. load all plugins
        await this.loadAllPlugins();
        // 3.1 安装内置插件（Navidrome / 网易云 / QQ）
        await this.ensureBuiltinPlugins();
        this.inited = true;
    }

    /** 从 res/builtin-plugins 安装内置音源插件 */
    private async ensureBuiltinPlugins() {
        const candidates = [
            path.resolve(process.resourcesPath || "", "res", "builtin-plugins"),
            path.resolve(app.getAppPath(), "res", "builtin-plugins"),
            path.resolve(__dirname, "../../../res/builtin-plugins"),
            path.resolve(process.cwd(), "res", "builtin-plugins"),
        ];
        let dir = "";
        ...
        try {
            ...
            for (const file of files) {
                try {
                    ...
                    await this.installPluginFromRawCodeImpl(code);
                } catch (e: any) {
                    // 已安装同名/同 hash 时静默忽略
                    logger.logError("install builtin plugin failed", e);
                }
            }
            await this.loadAllPlugins();
            this.syncPlugins();
        } catch (e: any) {
            logger.logError("ensureBuiltinPlugins error", e);
        }
    }
